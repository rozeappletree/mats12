"""Score trained control probes on a novel, unseen held-out dataset.

Loads the checkpoint(s) written by train_control_probe.py under --checkpoint_dir
...
Like train_control_probe.py, no "I think the {attribute} of this user is" prompt is appended --
the cached activation is the last token of the user's own final message, matching what the
...
Usage:
    python src/test_control_probe.py --checkpoint_dir probe_checkpoints/control_probe/regular_gullibility_170_custom_split --test_dirs datasets_hard_333
    python src/test_control_probe.py --checkpoint_dir probe_checkpoints/control_probe/regular_gullibility_170_custom_split --test_dirs datasets_defense_484 --attributes gullibility --layers 20
...
def main():
    parser = build_test_arg_parser(__doc__, default_probe_dir="probe_checkpoints/control_probe")
    args = parser.parse_args()
    evaluate(args, probe_type="control")
