"""
Synthetic dataset generation via Qwen 3.7 Plus (alibaba/qwen3.7-plus, served
through minirouter.sh) — gullible vs. non-gullible human defenses of
TruthfulQA-style claims, over the HARD-NEGATIVES split.

Input: data/finetune/split/hard_negatives_333.json (333 questions, each with
...
This is scripts/gen_defense_data484.py with the input/output swapped for the
"hard negatives" split (hard_negatives_333.json, same schema as
non_hard_negatives_484.json — question / best_answer / correct_answers /
incorrect_answers / category / type all line up field for field), and the
model switched to Qwen 3.7 Plus on minirouter.sh, reached through the same
OpenAI-compatible client (BASE_URL + OpenAI SDK) as the 484 script. Every
other step — prompting, validation, resumability, progress display — is
unchanged.

Usage:
    # the full run over all 333 questions
    python scripts/gen_hard_data333.py
    ...
    python scripts/gen_hard_data333.py --limit 20
"""
...
# zai/glm-5.3-flash was tried first but is impractically slow for this prompt
# on this gateway (~13 tok/s, 10+ min per call even with reasoning disabled).
# alibaba/qwen3.7-plus completes the same prompt in ~40s.
MODEL_NAME = "alibaba/qwen3.7-plus"
BASE_URL = "https://api.minirouter.sh/v1"

# Pushed toward maximum diversity across the 6 conversations per call, on top
# of the prompt-level diversity seeding (CONVERSATION_ANGLES / USER_VOICES).
# Bad or incoherent outputs are still caught by validate_conversation, so
# erring high here just costs a few more dropped conversations, not bad data.
# top_k is NOT set: this gateway's billing plan rejects it outright (400
# "Field 'top_k' is not supported on this billing snapshot").
TEMPERATURE = 1.2
TOP_P = 0.98

# Placeholder pricing carried over from an earlier model — minirouter doesn't
# expose per-model pricing via the API. Used for the live spend readout only;
# not authoritative, check the minirouter dashboard for actual billing.
PRICE_IN_PER_MTOK = 0.158
PRICE_OUT_PER_MTOK = 0.525

SEED = 75241239

DEFAULT_INPUT = "data/finetune/split/hard_negatives_333.json"
DEFAULT_OUTPUT_DIR = "datasets_hard_333"
...
def run_one_question(
    ...
    for attempt in range(1, MAX_CALL_RETRIES + 1):
        ...
        try:
            ...
            prog.failures += 1
            prog.log(f"  [q{qidx}] attempt {attempt} rate limited: {e}")
            parsed = None
        except openai.APIStatusError as e:
            prog.failures += 1
            if e.status_code < 500:
                # A 4xx other than 429 (bad model name, bad request, auth,
                # permissions, ...) won't fix itself on retry.
                prog.log(f"  [q{qidx}] attempt {attempt} non-retryable "
                         f"{e.status_code} error: {e}; giving up on this question")
                parsed = None
                break
            prog.log(f"  [q{qidx}] attempt {attempt} server error: {e}")
            parsed = None
        except openai.APIConnectionError as e:
            prog.failures += 1
            prog.log(f"  [q{qidx}] attempt {attempt} connection error: {e}")
            parsed = None
...
def main():
    ...
    parser.add_argument(
        ...
        help="Path to the hard_negatives-style questions JSON.",
    )
    parser.add_argument(
    ...
    api_key = os.getenv("MINIROUTER_KEY") or os.getenv("OPUSKEY") or os.getenv("ANTHROPIC_API_KEY")
    if not api_key:
        raise SystemExit(
            "No API key found. Set MINIROUTER_KEY, OPUSKEY, or ANTHROPIC_API_KEY "
