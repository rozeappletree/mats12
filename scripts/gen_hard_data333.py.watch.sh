#!/usr/bin/env bash
# View progress of the detached scripts/gen_hard_data333.py run.
#
#   ./scripts/gen_hard_data333.py.watch.sh        one snapshot
#   ./scripts/gen_hard_data333.py.watch.sh -w      refresh every 30s
#   ./scripts/gen_hard_data333.py.watch.sh -f      tail the raw log
#
# The run writes to datasets_hard_333/run.log, but its live progress bar is
...
snapshot() {
    ...
    echo "=============================================================="
    echo " gen_hard_data333   $status   $(date '+%Y-%m-%d %H:%M:%S')"
    echo "=============================================================="
